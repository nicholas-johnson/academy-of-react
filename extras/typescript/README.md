...
function WizardList() {
  ...
  if (state.status === 'success') {
    return (
      <ul>
        {state.data.map(w => <li key={w.name}>{w.name}</li>)}
      </ul>
    )
  }
...
**This is an extras module** — not part of the main course flow. TypeScript is introduced in Module 10 (useRef), but this reference covers fundamentals more deeply. Use it as a reference or primer before Phase 3.
