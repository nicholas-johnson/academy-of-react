...
function WizardList() {
  ...
  return (
    <div>
      {wizards.map(w => <div key={w.id}>{w.name}</div>)}
...
function WizardList() {
  ...
  return (
    <div>
      {wizards.map(w => <div key={w.id}>{w.name}</div>)}
      <button onClick={() => dispatch(wizardSlice.actions.addWizard({ id: 1, name: 'Harry' }))}>
        Add Wizard
      </button>
    </div>
  )
...
const WizardList = observer(() => {
  return (
    <div>
      {store.wizards.map(w => <div key={w.id}>{w.name}</div>)}
      <button onClick={() => store.addWizard({ id: 1, name: 'Harry', power: 100 })}>
        Add Wizard
      </button>
